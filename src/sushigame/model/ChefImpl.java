...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double balance, food_eaten, food_spoiled;
	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				IngredientPortion[] contents = plate.getContents().getIngredients();
				for (IngredientPortion portion : contents) {
					food_eaten += portion.getAmount();
				}
			...
			if (plate.getChef() == this) {
				IngredientPortion[] contents = plate.getContents().getIngredients();
				for (IngredientPortion portion : contents) {
					food_spoiled += portion.getAmount();
				}
	...
	public double foodEaten() {
		//		double eaten = 0.0;
		//
		//		for (HistoricalPlate plate : getPlateHistory()) {
		//			if (plate != null) {
		//				if (!plate.wasSpoiled()) {
		//					IngredientPortion[] contents = plate.getContents().getIngredients();
		//					for (IngredientPortion portion : contents) {
		//						eaten += portion.getAmount();
		//					}
		//				}
		//			}
		//		}
